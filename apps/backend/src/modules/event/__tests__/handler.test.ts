...
function resetServiceMocks() {
  ...
  (eventService.createEvent as Mock<typeof eventService.createEvent>).mockReset();
  (eventService.voteOnEvent as Mock<typeof eventService.voteOnEvent>).mockReset();
  (eventService.getEventResults as Mock<typeof eventService.getEventResults>).mockReset();
}
...
describe('Event handlers', () => {
  ...
  describe('GET /api/v1/events', () => {
    it('returns an empty events array', async () => {
      (eventService.getEvents as Mock<typeof eventService.getEvents>).mockResolvedValue([]);
  ...
  describe('GET /api/v1/events/:id', () => {
    it('throws when the event is not found', async () => {
      ; (eventService.getEvent as Mock<typeof eventService.getEvent>).mockResolvedValue(null)
    ...
    it('returns the event when found', async () => {
      const event = { id: 'evt-1', name: 'Birthday', dates: [], votes: [] }
        ; (eventService.getEvent as Mock<typeof eventService.getEvent>).mockResolvedValue(event)
  ...
  describe('POST /api/v1/events', () => {
    it('creates an event and returns it', async () => {
      const created = { id: 'evt-2', name: 'New Event', dates: [], votes: [] }
        ; (eventService.createEvent as Mock<typeof eventService.createEvent>).mockResolvedValue(created)
  ...
  describe('POST /api/v1/events/:id/vote', () => {
    it('records a vote and returns the updated event', async () => {
      const updated = { id: 'evt-1', name: 'Birthday', dates: [], votes: [] }
        ; (eventService.voteOnEvent as Mock<typeof eventService.voteOnEvent>).mockResolvedValue(updated)
