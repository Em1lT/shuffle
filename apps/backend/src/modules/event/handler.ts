...
import { getEventsRoute, getEventRoute, postEventRoute, postEventVoteRoute, getSuitableDatesRoute } from './routes'

const app = new OpenAPIHono()

export const getEventsHandler: RouteHandler<typeof getEventsRoute> = async (ctx) => {
  const events = await eventService.getEvents()
  return ctx.json({ events })
}

export const getEventHandler: RouteHandler<typeof getEventRoute> = async (ctx) => {
...
export const getSuitableDatesHandler: RouteHandler<typeof getSuitableDatesRoute> = async (ctx) => {
  const { id } = ctx.req.valid('param')
