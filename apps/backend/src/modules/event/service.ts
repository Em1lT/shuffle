...
export const createEvent = async (data: { name: string; dates: Date[] }) => {
	const eventId = await db.transaction(async (tx) => {
		...
		const eventDateItems = data.dates.map((date) => ({
			eventId: created.id,
			date,
		}));
	...
	const eventId = await db.transaction(async (tx) => {
		...
		await Promise.all(
			data.votes.map(async (date) => {
				...
				await tx
					.insert(eventVotes)
					.values({ eventId: event.id, dateId, userId, name: data.name });
			}),
		);
...
export const getEventResults = async (id: string) => {
	const participantCount = sql`(SELECT count(DISTINCT ${eventVotes.userId}) FROM ${eventVotes} WHERE ${eventVotes.eventId} = ${events.id})`;
